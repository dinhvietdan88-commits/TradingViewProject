    ...
    def run_ingest():
        try:
            ...
            try:
                ...
                if res.returncode == 0:
                    log.info(
                        f"[Semantic Ingestion] Successfully ingested event ({category}): {text[:100]}..."
                    )
                else:
                    log.warning(
                        f"[Semantic Ingestion] Ingest command failed with code {res.returncode}: {res.stderr.strip()}"
                    )
                    run_fallback = True
            except (PermissionError, FileNotFoundError, OSError) as exec_err:
                log.info(
                    f"[Semantic Ingestion] Executing angati.exe failed ({exec_err}). Using direct SQLite fallback."
                )
                run_fallback = True
            ...
            if run_fallback:
                try:
                    ...
                    metadata_json = json.dumps(
                        {"category": category, "source": "fallback_ingest"}
                    )
                    cursor.execute(
                    ...
                    conn.close()
                    log.info(
                        f"[Semantic Ingestion] Fallback SQLite successfully ingested event ({category}): {text[:100]}..."
                    )
                except Exception as db_err:
                    log.error(
                        f"[Semantic Ingestion] Fallback SQLite ingestion failed: {db_err}"
                    )
