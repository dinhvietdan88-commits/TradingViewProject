...
async def run_case_1():
    ...
    mock_place_order = AsyncMock(return_value={
        "orderId": "MOCK-ENTRY-WEEX",
        "executedQty": "0.001",
        "cummulativeQuoteQty": "60.0",
        "status": "FILLED"
    })
    mock_place_oco = AsyncMock(return_value={
        "orderListId": "MOCK-OCO-WEEX"
    })
    
    with patch.object(adapter, "place_market_order", mock_place_order), \
        ...
        log.info("[+] Passed 1.1: BTC volume clamped to minimum 0.001 BTC successfully.")
        ...
        log.info("[+] Passed 1.3: Other assets value clamped to minimum $5.00 successfully.")
        ...
        log.info("[+] Passed 1.4: Other assets value clamped to maximum $10.00 successfully.")
...
async def run_case_2():
    ...
    async with database.aiosqlite.connect(config.DB_PATH) as db:
        await db.execute("UPDATE trades SET pnl = -12.50 WHERE order_id = 'MOCK-LOSS-001'")
        await db.commit()
    ...
    async with database.aiosqlite.connect(config.DB_PATH) as db:
        ...
        async with db.execute("SELECT * FROM trades WHERE signal_id = ?", (target_sig_id,)) as cursor:
            trade_row = await cursor.fetchone()
            assert trade_row is not None
            assert trade_row["status"] == "FAILED"
            assert "exceeds cap" in trade_row["error_message"]
            
        async with db.execute("SELECT processed FROM signals WHERE id = ?", (target_sig_id,)) as cursor:
    ...
    log.info("[+] Passed 2.0: Daily Loss Cap blocked trade execution and set status correctly.")
...
async def run_case_3():
    ...
    sig_1 = await database.insert_signal(symbol="BTCUSDT", action="buy", price=65000.0, quote_qty=100.0, source_ip="127.0.0.1", payload={})
    sig_2 = await database.insert_signal(symbol="BTCUSDT", action="buy", price=65000.0, quote_qty=100.0, source_ip="127.0.0.1", payload={})
    
    await database.insert_trade(signal_id=sig_1, symbol="BTCUSDT", side="BUY", order_id="DD-1", status="FILLED", exchange="weex")
    await database.insert_trade(signal_id=sig_2, symbol="BTCUSDT", side="BUY", order_id="DD-2", status="FILLED", exchange="weex")
    
    # Set created_at to 48 hours ago so they contribute to rolling drawdown but NOT to daily loss!
    async with database.aiosqlite.connect(config.DB_PATH) as db:
        await db.execute("UPDATE trades SET pnl = 100.0, created_at = datetime('now', '-48 hours') WHERE order_id = 'DD-1'")
        await db.execute("UPDATE trades SET pnl = -60.0, created_at = datetime('now', '-48 hours') WHERE order_id = 'DD-2'")
        await db.commit()
...
async def run_case_4():
    ...
    weex_client.exchange_name = "weex"
    weex_client.exchange_id = "weex"
    weex_client.execute_smart_order = AsyncMock(side_effect=ExchangeError(ExchangeErrorCategory.CONNECTION_ERROR, "Weex API connection refused"))
    
    bybit_client = AsyncMock()
    bybit_client.exchange_name = "bybit"
    bybit_client.exchange_id = "bybit"
    bybit_client.execute_smart_order = AsyncMock(return_value=MagicMock(
        success=True,
        dry_run=False,
        side="BUY",
        symbol="BTCUSDT",
        entry_order={"orderId": "FALLBACK-BYBIT-001", "status": "FILLED", "executedQty": "0.001", "cummulativeQuoteQty": "65.0"},
    ...
    async def on_exec(ev):
        ...
        log.info(f"Received TradeExecuted event: order_id={ev.order_id}, exchange={ev.exchange}")
    ...
    log.info("[+] Passed 4.0: Weex connection failure triggered fallback to Bybit successfully.")
    results["Case 4: Fallback Routing"] = "PASSED"

async def run_case_5():
    log.info("--- TEST CASE 5: Orphan Position Prevention (Entry Success, OCO Exit Failure) ---")
    ...
    mock_place_oco = AsyncMock(side_effect=Exception("OCO Order Rejected by exchange API"))
    ...
    with patch.object(adapter, "place_market_order", mock_place_order), \
        ...
        log.info("[+] Passed 5.0: Entry order successfully cancelled after OCO exit failure.")
