...
check(
    "T3  /auth/logout               (no session->redir)",
    f"{BASE}/auth/logout",
    [200, 302],
)
check(
    "T8  /static/login.html         (static asset)", f"{BASE}/static/login.html", [200]
)
check(
    "T9  /api/system/status         (public endpoint)",
...
check(
    "T4  /auth/callback?code=       (empty->400/401)",
    f"{BASE}/auth/callback?code=",
    [400, 401, 422],
)
check(
    "T5  /auth/callback (invalid)",
    f"{BASE}/auth/callback?code=deadbeef00000000deadbeef00000000",
    [400, 401],
)
