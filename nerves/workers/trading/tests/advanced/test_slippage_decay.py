...
class TestSlippageDecay:
    ...
    def test_slippage_performance_decay(self):
        ...
        if not os.path.exists(vbs_replay_db_path) or not os.path.exists(signals_db_path):
        ...
        for slip, res in results.items():
            print(f"Slippage: {slip*100:.2f}% | Trades: {res['trades']} | Net P&L: {res['total_pnl']:.2f} USDT | PF: {res['profit_factor']:.2f}")
