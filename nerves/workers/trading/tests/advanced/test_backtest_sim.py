...
async def test_backtest_pnl_sl_tp_logic(mock_bus, mocker):
    ...
    SmartOrderResult = namedtuple(
        "SmartOrderResult",
        ["success", "entry_order", "oco_order", "risk", "error", "dry_run"],
    )
    ...
    mocker.patch(
        "exchanges.router.ExchangeRouter.resolve_exchange", return_value=mock_adapter
    )
    mocker.patch(
        "engine.trade_engine.get_symbol_config",
        return_value={
            "risk_pct": 0.02,
            "stop_loss_pct": 0.10,
            "atr_sl_mul": 2.0,
    ...
    print(
        f"Backtest Simulation Passed: Simulated 100 ticks. Result: {result_status}, PnL: +{pnl:.2f}"
    )
