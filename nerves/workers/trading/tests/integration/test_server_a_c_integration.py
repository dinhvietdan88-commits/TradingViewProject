...
async def get_signal_db_status(queue_id: int):
    ...
    async with aiosqlite.connect(vbs_config.DB_PATH) as db:
        async with db.execute(
            "SELECT status, ack_status FROM signal_queue WHERE id = ?", (queue_id,)
        ) as cur:
    ...
    async def __aenter__(self):
        if self.method == "GET":
            response = await self.client.get(
                self.path, params=self.params, headers=self.headers
            )
        elif self.method == "POST":
            response = await self.client.post(
                self.path, json=self.json_data, headers=self.headers
            )
        else:
...
class MockAiohttpSession:
    ...
    def get(self, url, params=None, headers=None, timeout=None):
        ...
        return MockResponseContext(
            self.vbs_app, "GET", path, params=params, headers=headers
        )
    ...
    def post(self, url, json=None, headers=None, timeout=None):
        ...
        path = urlparse(url).path
        return MockResponseContext(
            self.vbs_app, "POST", path, json_data=json, headers=headers
        )
...
@pytest.mark.parametrize(
    "confidence, mode, expected_approval, expected_hold",
    [
...
async def test_confidence_edge_cases(
    test_db, worker, confidence, mode, expected_approval, expected_hold
):
    """
    ...
    if mode == "ai":
        ...
        patch_conf = patch.object(
            worker, "_extract_confidence", return_value=confidence
        )
        patch_algo = patch.object(
            worker, "_algorithmic_analysis", return_value=("Algo advice", confidence)
        )
    else:
        patch_available = patch(
...
async def test_fallback_routing_and_recovery(test_db):
    ...
    session.post_calls = []
    session.post_responses = {
        "http://local-execute": [Exception("Connection refused")],
    ...
    session.post_responses = {
        "http://local-execute": [
            FakeResponse(
                status=200, json_data={"success": True, "order_id": "LOCAL-RECOVERED"}
            )
        ]
    }
