...
class TestVpsAnalyzerRagContext(unittest.IsolatedAsyncioTestCase):
    async def test_vps_analyzer_fetches_ohlcv_and_injects_stats(self):
        ...
        mock_advice_func = AsyncMock(
            return_value="APPROVED: The VCP pattern is strong."
        )
        ...
        with (
            ...
            patch(
                "capture_client.get_capture_client", return_value=mock_capture_client
            ),
            patch(
                "workers.vps_analyzer.rag.generate_trading_advice", mock_advice_func
            ) as mock_gen_advice,
            patch(
            ...
            mock_capture_client.fetch_ohlcv.assert_called_once_with(
                "BTCUSDT", timeframe="D", limit=365
            )
