...
async def test_morning_brief_fallback_to_mplfinance():
    ...
    with patch("brief.get_watchlist", return_value=["BTCUSDT"]) as mock_watchlist, \
         ...
         patch("brief.generate_trading_advice", new_callable=AsyncMock, return_value="Mock advice") as mock_advice, \
        ...
        with patch("capture_client.PythonCaptureClient.capture_screenshot", new_callable=AsyncMock, return_value=mock_capture_res) as mock_local_capture, \
            ...
            try:
                ...
                assert Path(brief["screenshot"]) == Path("mock_screenshots/brief_BTCUSDT.png")
