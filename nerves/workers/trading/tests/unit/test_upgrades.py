...
async def test_regime_switcher_trending():
    ...
    with patch(
        "engine.regime_switcher.fetch_candles_with_retry",
        AsyncMock(return_value=mock_candles),
    ):
...
async def test_bear_end_tactical_sizing_breakout():
    ...
    mock_cursor.fetchone = AsyncMock(
        return_value={"action": "bo", "payload": '{"action": "bo"}'}
    )
    ...
    with (
        ...
        patch(
            "analysis.fetch_candles_with_retry", AsyncMock(return_value=mock_candles_1h)
        ),
        patch(
            "engine.regime_switcher.get_market_regime",
            AsyncMock(return_value="TRENDING"),
        ...
        mock_db.get_risk_settings = AsyncMock(
            return_value={
                "state": "CLOSED",
...
async def test_safe_mode_halves_sizing():
    ...
    mock_cursor.__aexit__ = AsyncMock(return_value=None)
    mock_cursor.fetchone = AsyncMock(
        return_value={"action": "buy", "payload": '{"action": "buy"}'}
    )
...
async def test_vision_sdk_fallback_parsing():
    ...
    with (
        patch("rag._collection", None),
        patch(
            "httpx.AsyncClient.post", AsyncMock(return_value=mock_response)
        ) as mock_post,
    ):
        result = await _analyze_chart_sdk_fallback(
            "BTCUSDT", {"trend_template_score": 8}
        )
