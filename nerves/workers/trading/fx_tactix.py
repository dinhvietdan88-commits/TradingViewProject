...
async def _call_provider(prompt: str) -> tuple[str, str]:
    ...
    if provider == "claude_cli":
        try:
            ...
            log.warning(
                f"fx-tactix: claude_cli failed ({e}); falling back to anthropic SDK"
            )
            provider = "anthropic"
    ...
    if provider == "anthropic":
        ...
        if importlib.util.find_spec("anthropic") is None or not getattr(
            config, "ANTHROPIC_API_KEY", None
        ):
            raise HTTPException(
                status_code=503, detail="Anthropic SDK/API key chưa cấu hình"
            )
        import anthropic
    ...
    if provider == "gemini":
        ...
        if importlib.util.find_spec("google.genai") is None or not getattr(
            config, "GEMINI_API_KEY", None
        ):
            raise HTTPException(
                status_code=503, detail="Gemini SDK/API key chưa cấu hình"
            )
...
async def generate(
    req: GenerateRequest, authorization: Optional[str] = Header(None)
) -> GenerateResponse:
    ...
    if req.style in ("strategy", "indicator", "alert"):
        ...
        if req.save and pine_code:
            ...
            log.info(
                f"fx-tactix: saved {file_path} ({len(pine_code)} chars, provider={provider})"
            )
        elif not pine_code:
            log.warning(
                f"fx-tactix: provider returned no Pine block for style={req.style}"
            )
