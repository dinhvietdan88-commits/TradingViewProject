...
def install_pre_commit_hook():
    ...
    if not os.path.exists(git_hooks_dir):
        print(f"Error: .git/hooks directory not found at {git_hooks_dir}. Make sure you are in a git repository.", file=sys.stderr)
        sys.exit(1)
...
"""
    ...
    try:
        ...
        print(f"[Pre-commit Security] Pre-commit security hook successfully installed at: {pre_commit_path}")
        print("Hook will run automatically on 'git commit' whenever Python files are modified.")
