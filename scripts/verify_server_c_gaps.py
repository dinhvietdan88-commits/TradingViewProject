...
def main():
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            for field, expected_type in fields.items():
                assert field in data, f"Field '{field}' missing in health check response"
                val = data[field]
                assert isinstance(val, expected_type), f"Field '{field}' type is {type(val)}, expected {expected_type}"
                if expected_type is str and field.startswith("liveness_status"):
                    assert val in ("healthy", "unhealthy"), f"Field '{field}' value '{val}' not in ('healthy', 'unhealthy')"
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            assert "# HELP liveness_status_server_a" in body, "Missing HELP liveness_status_server_a"
            assert "liveness_status_server_a" in body, "Missing liveness_status_server_a gauge"
            assert "# HELP disk_usage_pct" in body, "Missing HELP disk_usage_pct"
            assert "disk_usage_pct" in body, "Missing disk_usage_pct gauge"
            assert "circuit_breaker_state" in body, "Missing circuit_breaker_state gauge"
        ...
        with urllib.request.urlopen(req_json, timeout=5) as response:
            ...
            for field in expected_fields:
                assert field in data, f"Field '{field}' missing in JSON metrics response"
                assert isinstance(data[field], (int, float)), f"Field '{field}' is not a numeric type"
    ...
    try:
        ...
        print_success(f"Daemon shutdown gracefully with exit code 0.")
        ...
        print(f"Analyzed {total_log_lines} log lines. Found {json_log_count} valid structured JSON log lines.")
