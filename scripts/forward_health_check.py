...
def main() -> None:
    ...
    parser.add_argument("--url", default="http://localhost:5000", help="Server C API URL")
    ...
    print(f"{DIM}  Time: {datetime.now(tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}{RESET}")
    ...
    check("forward_trades.db exists", FWD_DB.exists(), str(FWD_DB.relative_to(ROOT)) if FWD_DB.exists() else "NOT FOUND")
    ...
    is_relative = fwd_db_path and not (fwd_db_path.startswith("C:") or fwd_db_path.startswith("/Users") or fwd_db_path.startswith("/home/") and "Users" in fwd_db_path)
    check("FORWARD_DB_PATH is relative", bool(is_relative), f"= {fwd_db_path}")
    check("FORWARD_TEST_ENABLED=true", env.get("FORWARD_TEST_ENABLED") == "true", env.get("FORWARD_TEST_ENABLED", "NOT SET"))
    check("FORWARD_TEST_INITIAL_CAPITAL set", "FORWARD_TEST_INITIAL_CAPITAL" in env, env.get("FORWARD_TEST_INITIAL_CAPITAL", "NOT SET"))
    check("WEBHOOK_SECRET set", bool(secret and len(secret) > 8), "***" + secret[-4:] if len(secret) > 4 else "NOT SET")
    ...
    if FWD_DB.exists():
        ...
        sig_count = conn.execute("SELECT COUNT(*) FROM signals WHERE mode='FORWARD'").fetchone()[0]
        check("Has FORWARD signals", sig_count > 0, f"count = {sig_count}")

        if sig_count > 0:
            seq = conn.execute("SELECT seq FROM sqlite_sequence WHERE name='signals'").fetchone()
            ...
            if latest:
                check(f"Latest signal has mode=FORWARD", latest[2] == "FORWARD",
                      f"id={latest[0]:,} {latest[1]} mode={latest[2]}")
                check(f"Latest signal id ≥ 1,000,000", latest[0] >= 1_000_000, f"id = {latest[0]:,}")
    ...
    is_healthy = status == 200 and ("ok" in body.lower() or "healthy" in body.lower() or "status" in body.lower())
    check("Health endpoint responds", is_healthy, f"HTTP {status}" if status else "Connection refused", warn=not is_healthy)
    ...
    if is_healthy:
        ...
        # Smoke test webhook
        s3, b3 = http_post(f"{args.url}/webhook", {
            "secret": secret,
            "symbol": "BTCUSDT",
            "action": "buy",
            "price": "67000",
        ...
        check("POST /webhook (mode=FORWARD)", webhook_ok, f"HTTP {s3}" + (f": {b3[:60]}" if args.verbose else ""), warn=not webhook_ok)
    else:
        check("API tests skipped", True, "Server offline — expected if running locally", warn=True)
    ...
    if pine_lib.exists():
        ...
        check("vbs_webhook_lib.pine has mode param", "string mode" in content and '"mode"' in content,
              "mode param in build_payload")
        check("Default mode = FORWARD", 'mode = "FORWARD"' in content, 'string mode = "FORWARD"')
        check("a007_mis_webhook.pine exists", (ROOT / "pine/v2/a007_mis_webhook.pine").exists())
        check("supertrend_webhook.pine exists", (ROOT / "pine/v2/supertrend_webhook.pine").exists())
    else:
