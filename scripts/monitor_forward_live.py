...
def get_stats(conn: sqlite3.Connection) -> dict:
    ...
    cur = conn.execute(
        "SELECT COUNT(*) as total FROM signals WHERE mode='FORWARD'"
    )
...
def watch(db_path: Path, interval: int, api_url: Optional[str]) -> None:
    ...
    while True:
        try:
            ...
            if not db_path.exists():
                sys.stdout.write(f"\r  ⏳ Waiting for DB... ({iteration * interval}s elapsed)")
                sys.stdout.flush()
                time.sleep(interval)
                continue
            ...
            if last_id == 0 and stats["rows"]:
                ...
                print(f"  {DIM}Baseline: {stats['total']} existing signals (latest id={last_id}){RESET}")
...
Examples:
  ...
  python scripts/monitor_forward_live.py --db nerves/workers/trading/forward_trades.db
    ...
    parser.add_argument("--api", type=str, default=None, help="Server C API URL (e.g. http://localhost:5000)")
    parser.add_argument("--db", type=str, default=None, help="Custom path to forward_trades.db")
    parser.add_argument("--db-only", action="store_true", help="DB-only mode (skip API)")
