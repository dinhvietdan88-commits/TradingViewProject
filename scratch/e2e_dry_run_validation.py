...
async def run_validation():
    ...
    payload = {
        ...
        "source": "strategy",
        "payload": {"sl": "135.00", "tp": "165.00"},
    }

    async with httpx.AsyncClient() as client:
        resp = await client.post("http://127.0.0.1:9099/ingest", json=payload)
        print(
            f"[E2E Validation] VBS /ingest Response: {resp.status_code} - {resp.json()}"
        )
    ...
    async with aiosqlite.connect(config.FORWARD_DB_PATH) as db:
        async with db.execute(
            "SELECT id, symbol, action, mode, vbs_queue_id FROM signals WHERE vbs_queue_id = ?",
            (queue_id,),
        ) as cur:
    ...
    print(
        f"[E2E Validation] Stored signal details: ID={sig_id}, Symbol={sym}, Action={act}, Mode={mode}, VBS_Queue_ID={vq_id}"
    )
