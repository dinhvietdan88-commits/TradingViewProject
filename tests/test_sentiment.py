...
async def test_lexicon_sentiment():
    ...
    assert (
        analyze_text_sentiment(
            "The market is looking bullish and strong, we expect breakout!"
        )
        > 0.0
    )
    assert (
        analyze_text_sentiment("Rallying upward toward new ATH, accumulate buy signal.")
        > 0.0
    )
    ...
    assert (
        analyze_text_sentiment(
            "Extreme panic as price dumps. Crash and heavy selloff expected."
        )
        < 0.0
    )
